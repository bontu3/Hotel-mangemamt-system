<html>
<head>
<title>gion hotel</title>
<link rel="stylesheet" href="exter.css">
</head>
<body>
 <div class="upper_body">
        <div class="upper_body_bg">
<div class="logo">
   <img src="g7.jpg" height="150px" width="1500px">
      <h1 class = "blinking"><center>  GION  HOTEL  </center> </h1>    
   </div>
<div class="menu-bar">
<ul>
    <li class="active"><a href="gion.php">HOME</a>
	</li>
	<li><a href="gallery.php">GALLERY</a></li>
    <li><a href="">CUSTEMER</a>
	<div class="sub-menu-1">
	  <ul>
	    <li><a href="gallery.php">VIEW </a></li>
        <li><a href="comment.php">COMMENT</a></li>	  
	 </div>
	</li>
	
    <li><a href="">BOOKNING</a>
	<div class="sub-menu-1">
	  <ul>
	    <li ><a href="guestroom.php">GUSTES ROOM</a></li>
		 <li><a href="partyhall.php">PARTY HALL</a></li>
		 <li><a href="weddinghall.php">WEDDING HALL</a></li>
	  </ul>
	 </div>
	</li>
<li><a href="">MANAGEMENT</a>
	<div class="sub-menu-1">
	  <ul>
	   	    <li><a href="STAFMANAGEMENT.php">STAF MANAGEMENT</a></li>
		
      
	  </ul>
	 </div>
	</li>
		<li><a href="contact.php">CONTACT US</a></li>
	<li><a href="about.php">ABOUT</a></li>
 </ul></div></div></div>
  <div class="middle_body">
        <div class="motto">
            <h1 class="blinking">Book Your Stay Now!</h1>
        </div>
        <div class="book now">
            <div class="room-samples">
                <div class="main-room-samples">
                    <img src="wd.jpg" alt="Snow" style="width:80%">
					<h1>Deluxe-Wedding-Hall</h1> 
					<br>
					<p><b>capacity : 300 person<b></P><br>
					<p><b> minimum cost: $15000<b></p><br><br><br><br>
                   	<button class="book-btn "><a href="reception.php">Book Now!</a></button><p></p>
					
                </div>
                <div class="main-room-samples">
                    <img src="w2.jpg" alt="Forest" style="width:80%">
					<h1>Deluxe-Wedding-Hall</h1> 
					<br>
					<p><b>capacity : 150 person<b></P><br>
					<p><b> minimum cost: $1100<b></p><br><br><br><br>
                   	<button class="book-btn "><a href="reception.php">Book Now!</a></button><p></p>
					
                </div>
                <div class="main-room-samples">
                    <img src="we.jpg" alt="Forest" style="width:80%">
					<h1>Deluxe-Wedding-Hall</h1> 
					<br>
					<p><b>capacity : 200 person<b></P><br>
					<p><b> minimum cost: $1300<b></p><br><br><br><br>
                   	<button class="book-btn "><a href="reception.php">Book Now!</a></button><p></p>
					
                </div>
            </div>
        </div>


    </div>
      <div class="lower_body">
        <div class="lower_body_align">
            <div class="lower-1">
                <h3 class="lower-head">Services</h3>
                Hotel Room<br> Car Service<br> Delicious Dishes
            </div>
            <div class="lower-2">
                <h3 class="lower-head">Address</h3>
                Adis abeba, Ethiopia <br> grandhotel@gmail.com <br> +251**********
            </div>
        </div>
    </div>
</body>
</html>
</body>
</html>